...
class WpdiscuzWalker extends Walker_Comment implements WpDiscuzConstants {
    ...
    public function start_el(&$output, $comment, $depth = 0, $args = array(), $id = 0) {
        ...
        if ($isClosed) {
            $output .= '<i class="fa fa-lock wpd-closed" aria-hidden="true" title="' . $this->optionsSerialized->phrases['wc_closed_comment_icon_title'] . '"></i>';
        }
        ...
        if (isset($args['comment_status']) && is_array($args['comment_status']) && in_array($comment->comment_approved, $args['comment_status'])) {
            ...
            if (isset($args['high_level_user']) && $args['high_level_user'] && !$comment->comment_parent) {
                ...
                $output .= '<span class="wc_close_btn wc-cta-button"><i class="fa fa-comments-o"></i><span class="wc_close_text">' . $closeText . '</span></span>';
            }
            $output .= '</div>';
